// Ego browser acceptance helper. Prefix only our two test storage keys; user data is untouched.
...
export async function prepare(page,{prefix=QA_PREFIX}={}){
  const {identifier}=await page.cdp('Page.addScriptToEvaluateOnNewDocument',{source:QA_SCRIPT.replaceAll(QA_PREFIX,prefix)});
  try{await page.reload();await page.waitForFunction(()=>Boolean(window.planetDiagnostics),undefined,{timeout:10000});}
