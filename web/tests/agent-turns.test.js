...
test('robotaxi original answers retain source questions and new conditions',()=>{
 const output=demoTurn(answers[1],answers[0],2,[scenarios[0].text]);
 assert.ok(output.includes(scenarios[0].text));assert.ok(output.includes(answers[0].question));assert.ok(output.includes(answers[1].question));assert.ok(!output.includes('undefined'));
});
test('all nine topics provide six source-grounded local turns without leaking robotaxi',()=>{
 assert.equal(catalog.length,9);
 for(const topic of catalog){
  assert.equal(topic.scenarios.length,3);assert.ok(topic.answers.length>=3);
  for(let i=0;i<6;i++){
   ...
   if(topic.id!=='robotaxi'&&i>=2)assert.ok(!/转岗|无人车|扩区门槛/.test(text));
   if(i>=2)assert.ok(text.includes(topic.scenarios[0].text));
  }
 }
});
test('all displayed answers are complete originals with verified provenance',()=>{
 for(const t of catalog)for(const n of t.answers){
  assert.equal(n.exampleRelation,'unknown');
  assert.equal(n.sourceKind,'zhihu');assert.equal(n.contentStatus,'full');assert.ok(n.contentHtml);assert.equal(n.claim,n.body);
