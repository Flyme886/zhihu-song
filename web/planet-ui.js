...
export function setupPlanet(api){
 ...
 <section id="cosmic-home" aria-label="我的星球，宇宙远景"><div class="home-copy"><p class="cosmic-kicker"><span></span> A WORLD THAT IS YOURS</p><h1>每一个想法，<br>都有引力。</h1><p class="home-description">你留下的观点，走过的分歧，<br>慢慢长成一颗属于自己的星球。</p><button id="enter-planet" class="cosmic-primary"><span>进入我的星球</span><span class="arrow-orbit" aria-hidden="true">↗</span></button><p id="home-memory" class="home-memory">从第一个想法开始。</p></div><button id="planet-orb-hit" aria-label="点击星球，进入我的星球"><span class="orb-caption"><i></i>我的星球<small>MY LITTLE UNIVERSE</small></span></button><div class="home-bottom"><span>让思考留下轨迹</span><span>01 <i></i> YOUR OWN ORBIT</span></div></section>
 ...
 function toast(message){$('#planet-toast').textContent=message;$('#planet-toast').hidden=false;clearTimeout(toastTimer);toastTimer=setTimeout(()=>$('#planet-toast').hidden=true,3400);}
 function applyView(next){
  ...
  if(next!=='planet')closePanel(false);audio.setScene(next);updateSound();
 }
 ...
 async function handleRoute(){
  ...
  }else{if(view==='world')api.leaveWorld();audio.cancelCues();applyView('home');$('#enter-planet').focus();}
 }
 async function enter(){
  ...
  api.renderer()?.setFocus(null);travel=0;ringPlayed=false;travelDuration=reduced?.2:api.store.setting('visitedPlanet')?2.5:7;
 ...
 function focusSection(section,itemId){
  route={view:'planet',section,itemId};api.renderer()?.setFocus(section);focusWait=reduced?.01:1;panelReady=false;$('#memory-panel').hidden=true;$('#planet-surface').dataset.focused='true';document.body.dataset.reading='false';audio.cue('focus');sections.forEach(s=>document.querySelector(`[data-section="${s}"]`).setAttribute('aria-current',s===section?'page':'false'));
 }
 ...
 function renderPanel(){
  ...
  if(item){
   ...
   if(section==='opinions'){
    ...
    const labels={keep:'保留原判断',supplement:'补充原判断',changed:'改变原判断'};content.append(paragraph(item.note||'尚未补充个人结论。',labels[item.change]||'我现在的想法'),paragraph(item.common||'尚未确认共同点。','共同点'),paragraph([item.source,item.target].map(n=>(n?.author||'')+'：'+(n?.claim||n?.body||'')).join('\n\n'),'各自的关注'),paragraph(item.additions?.join('\n')||'引用的经历与适用条件仍需进一步核实。','待核实与补充条件'),sources([item.source,item.target].filter(Boolean)));
    const transcript=element('details');transcript.append(element('summary','回看完整对谈'));for(const m of item.messages||[])transcript.append(paragraph(m.text,m.who));content.append(transcript,paragraph('Agent 发言是基于材料的延展，不代表原答主本人。'));
 ...
 function updateIndex(){index=buildHistory(api.store.data,api.catalog);const counts=Object.fromEntries(sections.map(s=>[s,index[s].length]));api.renderer()?.setCounts(counts);landmarks.forEach((b,i)=>b.querySelector('b').textContent=counts[sections[i]]);$('#home-memory').textContent=counts.opinions||counts.records?`${counts.opinions} 个想法，${counts.cases} 段来路，${counts.records} 次相遇。`:'从第一个想法开始。';$('#surface-subtitle').textContent=counts.opinions||counts.records?'想法会改变，走过的路会留下。':'星球已在这里，等待你的第一个想法。';}
 ...
 $('#enter-planet').addEventListener('click',enter);$('#planet-orb-hit').addEventListener('click',enter);$('#cosmic-my-planet').addEventListener('click',enter);$('#cosmic-explore').addEventListener('click',()=>{audio.unlock().then(updateSound);exploreTopic();});
 ...
 function updateSound(){const enabled=audio.settings.enabled;$('#sound-toggle').setAttribute('aria-pressed',String(enabled));$('#sound-toggle').setAttribute('aria-label',enabled?'关闭声音':'开启声音');$('#sound-label').textContent=enabled?(audio.ready?'声音已开启':'声音待开启'):'声音已关闭';document.body.dataset.sound=enabled&&audio.ready?'on':'off';}
 $('#sound-toggle').addEventListener('click',async()=>{audio.update({enabled:!audio.settings.enabled});if(audio.settings.enabled)await audio.unlock().then(updateSound);updateSound();});
 ...
 return {
  ...
  enter,navigate,showWorld(id){route={view:'world',topicId:id};lastRoute='#/world/'+id;audio.cancelTravel();applyView('world');},
  onSave(){api.renderer()?.burst();audio.cue('save');updateIndex();toast(api.store.failed?'修改暂留在本页，请导出备份。':'新的想法，已留在你的星球。');},
