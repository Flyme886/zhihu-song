...
export function meadowMaterial(wind){
 ...
 mat.onBeforeCompile=shader=>{
  ...
  shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
   ...
   float lawn=smoothstep(1.02,2.18,edge);
   vec3 green=mix(vec3(.075,.145,.055),vec3(.22,.29,.105),broad);
   vec3 earth=mix(vec3(.36,.282,.175),vec3(.53,.435,.285),grain(p*.65));
   float worn=(1.-smoothstep(0.,.8,track))*.065;
   float cloud=mix(.83,1.,smoothstep(.25,.68,grain(p*.035+vec2(meadowTime*.007,0.))));
   diffuseColor.rgb*=mix(earth+vec3(worn),green,lawn)*(.87+.18*fine+.07*speck)*cloud;
  `);
 };
 return mat;
}
...
export function wildflowerGeometry(){
 ...
 for(let petal=0;petal<8;petal++){
  ...
  const point=(t,side)=>{const r=.022+t*.13,w=Math.sin(t*Math.PI)*.043*side;return[Math.cos(angle)*r-Math.sin(angle)*w,.49+Math.sin(t*Math.PI)*.025+t*t*.052,Math.sin(angle)*r+Math.cos(angle)*w];};
  for(let k=0;k<3;k++){const a=point(k/3,-1),b=point((k+1)/3,-1),c=point((k+1)/3,1),d=point(k/3,1);verts.push(...a,...c,...b,...a,...d,...c);}
 }
 return geometry(verts);
}

export function wildflowerStemGeometry(){
 const cylinder=new THREE.CylinderGeometry(.005,.008,.49,3).translate(0,.245,0),stem=cylinder.toNonIndexed();
 const vertices=Array.from(stem.attributes.position.array);cylinder.dispose();stem.dispose();
 for(let i=0;i<2;i++){
  const side=i?1:-1,y=.18+i*.10;
  vertices.push(0,y,0,side*.07,y+.02,.028,side*.15,y+.085,0,0,y,0,side*.15,y+.085,0,side*.07,y+.02,-.028);
 }
...
export function buildUnderstory(scene,wind){
 ...
 let n=0;while(n<grass.instanceMatrix.count){
  const x=(rng()-.5)*96,z=(rng()-.5)*96-10;
  ...
  col.set([0x566e3e,0x73864d,0x90975c,0x637a41,0xa4a06c][Math.floor(rng()*5)]);grass.setColorAt(n++,col);
 }
 ...
 for(let i=0;i<420;i++){
  let x,z;do{x=(rng()-.5)*88;z=(rng()-.5)*88-8;}while(pathDistance(x,z)>1.6);
...
export function buildGrove(scene,wind){
 ...
 const leaf=geometry([0,0,0,-.10,.015,.16,0,.055,.29,0,0,0,0,.055,.29,.10,.015,.16,-.10,.015,.16,-.085,.025,.32,0,.015,.46,-.10,.015,.16,0,.015,.46,0,.055,.29,0,.055,.29,0,.015,.46,.085,.025,.32,0,.055,.29,.085,.025,.32,.10,.015,.16]);
 const leafMat=surface(0xffffff,{side:THREE.DoubleSide,emissive:0x5b662e,emissiveIntensity:.10});
 leafMat.onBeforeCompile=shader=>{
  Object.assign(shader.uniforms,{groveTime:wind.time,groveWind:wind.amount});
  shader.vertexShader='uniform float groveTime,groveWind;\n'+shader.vertexShader;
  shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>',`#include <begin_vertex>
   vec3 origin=instanceMatrix[3].xyz;
   transformed.x+=sin(groveTime*.9+origin.x*.6+origin.z*.4)*position.z*.18*groveWind;
   transformed.y+=sin(groveTime*1.7+origin.z)*position.z*.065*groveWind;`);
 };
 const foliage=new THREE.InstancedMesh(leaf,leafMat,TREE_POSITIONS.length*2800);
 ...
 for(const [x,z] of TREE_POSITIONS){
  ...
  const clusters=[];
  for(let b=0;b<6;b++){
   const a=b*2.4+rng()*.5,r=(1.2+rng()*1.1)*s,y=ground+(4.1+rng()*2.1)*s;
  ...
  for(let i=0;i<2800;i++){
   ...
   dummy.rotation.set((rng()-.5)*1.6,rng()*6.28,(rng()-.5)*1.3);dummy.scale.setScalar(.86+rng()*.7);dummy.updateMatrix();foliage.setMatrixAt(leafIndex,dummy.matrix);
   col.set([0x81914c,0xa8a45e,0xb8ae6c,0x698247,0xc6bb79,0x929655][Math.floor(rng()*6)]);foliage.setColorAt(leafIndex++,col);
  }
 }
