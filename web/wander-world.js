...
import {Walker,terrainHeight,pathDistance,memoryPosition,TREE_POSITIONS,OBSTACLES,hashString} from './wander-controller.js';
...
export class WanderWorld{
 constructor(renderer,camera){
  this.renderer=renderer;this.camera=camera;this.scene=new THREE.Scene();this.scene.fog=new THREE.Fog(0xdce4d3,75,240);this.walker=new Walker();this.width=1280;this.height=800;this.time=0;this.keys={};this.blocked=false;this.view='home';this.nearest=null;this.markers=[];this.assets={traveler:'loading',environment:'loading',landscape:'loading'};this.target=new THREE.Vector3();this.look=new THREE.Vector3();this.lastView=null;this.reduced=false;this.quality='high';this.listeners={};
  this.buildSky();this.buildTerrain();this.buildFlowers();this.buildTrees();this.buildProps();this.buildAvatar();this.loadAssets();
 }
 buildSky(){
  ...
  this.scene.add(new THREE.HemisphereLight(0xe3f4ff,0xa28746,2.4));const sun=new THREE.DirectionalLight(0xffe5b2,3.2);sun.position.set(-35,65,30);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);sun.shadow.camera.left=-48;sun.shadow.camera.right=48;sun.shadow.camera.top=38;sun.shadow.camera.bottom=-42;sun.shadow.camera.far=170;sun.shadow.bias=-.0005;sun.shadow.normalBias=.04;sun.target.position.set(0,0,-10);this.scene.add(sun,sun.target);this.sun=sun;
  const landscape=new THREE.TextureLoader().load('/assets/planet/daylight/alpine-dream.png',()=>{this.assets.landscape='ready';},undefined,()=>{this.assets.landscape='fallback';this.backdrop.visible=false;});landscape.colorSpace=THREE.SRGBColorSpace;
  this.backdrop=mesh(new THREE.PlaneGeometry(720,405),new THREE.MeshBasicMaterial({map:landscape,fog:false}),this.scene,0,20,-280);
  // The planet and its rings have real depth and move with the camera.
  const planetMat=new THREE.ShaderMaterial({vertexShader:'varying vec3 n,p;void main(){n=normalize(normalMatrix*normal);p=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',fragmentShader:'varying vec3 n,p;void main(){float b=.5+.5*sin(p.y*.49+sin(p.y*.95)*.7);vec3 c=mix(vec3(.63,.41,.27),vec3(.94,.74,.49),b);c=mix(c,vec3(.62,.76,.75),smoothstep(.82,.98,b)*.55);float l=.37+.63*max(0.,dot(normalize(n),normalize(vec3(-.6,.65,1.))));gl_FragColor=vec4(c*l,1.);}'});
  this.planet=mesh(new THREE.SphereGeometry(19,64,48),planetMat,this.scene,57,33,-172);
  ...
  this.ring=mesh(ring,ringMat,this.scene,57,33,-172);this.ring.rotation.set(1.06,.16,-.47);
 ...
 buildTerrain(){
  const geo=new THREE.PlaneGeometry(230,230,160,160);geo.rotateX(-Math.PI/2);const p=geo.attributes.position,c=new Float32Array(p.count*3),color=new THREE.Color(),rng=rand(562);
  for(let i=0;i<p.count;i++){const x=p.getX(i),z=p.getZ(i);p.setY(i,terrainHeight(x,z));const path=pathDistance(x,z)<1.5;color.set(path?0xba9b6c:0x596d3c);color.multiplyScalar(.85+rng()*.2);c.set([color.r,color.g,color.b],i*3);}geo.setAttribute('color',new THREE.BufferAttribute(c,3));geo.computeVertexNormals();this.ground=mesh(geo,material(0xffffff,{vertexColors:true}),this.scene);this.ground.receiveShadow=true;
 ...
 buildFlowers(){
  ...
  function tri(a,b,c){vertices.push(...a,...b,...c);const v=new THREE.Vector3(...b).sub(new THREE.Vector3(...a)).cross(new THREE.Vector3(...c).sub(new THREE.Vector3(...a))).normalize();normals.push(...v,...v,...v);}
  for(let i=0;i<9;i++){const a=i/9*Math.PI*2,ca=Math.cos(a),sa=Math.sin(a),tx=-sa*.065,tz=ca*.065,base=[ca*.055,.59,sa*.055],left=[ca*.19+tx,.625,sa*.19+tz],tip=[ca*.31,.67,sa*.31],right=[ca*.19-tx,.625,sa*.19-tz];tri(base,left,tip);tri(base,tip,right);}
  const flowerGeo=new THREE.BufferGeometry();flowerGeo.setAttribute('position',new THREE.Float32BufferAttribute(vertices,3));flowerGeo.setAttribute('normal',new THREE.Float32BufferAttribute(normals,3));
  ...
  const count=16000,petals=new THREE.InstancedMesh(flowerGeo,sway(material(0xeaae25,{side:THREE.DoubleSide})),count),centers=new THREE.InstancedMesh(new THREE.SphereGeometry(.067,6,4).translate(0,.64,0),sway(material(0x71512e)),count),stems=new THREE.InstancedMesh(new THREE.CylinderGeometry(.013,.021,.61,4).translate(0,.305,0),sway(material(0x5e7244)),count);
  const matrix=new THREE.Matrix4(),dummy=new THREE.Object3D(),rng=rand(843),color=new THREE.Color();let j=0;
  while(j<count){const x=(rng()-.5)*112,z=(rng()-.5)*112-12;if(pathDistance(x,z)<1.8||OBSTACLES.some(o=>Math.hypot(x-o.x,z-o.z)<o.r+1))continue;const scale=.72+rng()*.78;dummy.position.set(x,terrainHeight(x,z),z);dummy.rotation.set((rng()-.5)*.2,rng()*6.28,(rng()-.5)*.2);dummy.scale.setScalar(scale);dummy.updateMatrix();matrix.copy(dummy.matrix);petals.setMatrixAt(j,matrix);centers.setMatrixAt(j,matrix);stems.setMatrixAt(j,matrix);color.set([0xeaaa23,0xe29820,0xf3c94b,0xd77928,0xefbd33][Math.floor(rng()*5)]);petals.setColorAt(j,color);j++;}
  for(const m of [petals,centers,stems]){m.frustumCulled=false;m.receiveShadow=true;this.scene.add(m);}this.flowers=[petals,centers,stems];
 }
 buildTrees(){
  const positions=TREE_POSITIONS;const rng=rand(832),dummy=new THREE.Object3D(),leaves=new THREE.InstancedMesh(new THREE.IcosahedronGeometry(1,1),material(0xd39535,{flatShading:false}),positions.length*86),trunks=new THREE.InstancedMesh(new THREE.CylinderGeometry(.10,.27,6.4,7),material(0x756448),positions.length);let n=0;
  const col=new THREE.Color();positions.forEach(([x,z],i)=>{const h=terrainHeight(x,z),s=.8+rng()*.55;dummy.position.set(x,h+2.8*s,z);dummy.rotation.set(0,rng()*3,.04);dummy.scale.set(s,s,s);dummy.updateMatrix();trunks.setMatrixAt(i,dummy.matrix);
   for(let j=0;j<86;j++){const a=rng()*6.28,r=Math.sqrt(rng())*3.2*s;dummy.position.set(x+Math.cos(a)*r,h+(4.7+rng()*2.8)*s,z+Math.sin(a)*r);dummy.rotation.set(rng()*3,rng()*3,rng()*3);dummy.scale.set(.38+rng()*.52,.35+rng()*.55,.4+rng()*.58);dummy.updateMatrix();leaves.setMatrixAt(n,dummy.matrix);col.set([0xc97732,0xd19d36,0xe3b443,0xbd6230,0x9f993f][Math.floor(rng()*5)]);leaves.setColorAt(n,col);n++;}
  });trunks.castShadow=true;leaves.castShadow=true;this.scene.add(trunks,leaves);this.leaves=leaves;
  // Fallen leaves are batched; they add scale at the feet without thousands of draw calls.
  const fallen=new THREE.InstancedMesh(new THREE.CircleGeometry(.095,5),material(0xcd8436,{side:THREE.DoubleSide}),520);for(let i=0;i<520;i++){const p=positions[i%positions.length],x=p[0]+(rng()-.5)*10,z=p[1]+(rng()-.5)*9;dummy.position.set(x,terrainHeight(x,z)+.025,z);dummy.rotation.set(-Math.PI/2,rng()*.2,rng()*6);dummy.scale.set(1+rng(),.6+rng()*.6,1);dummy.updateMatrix();fallen.setMatrixAt(i,dummy.matrix);}this.scene.add(fallen);
 }
 buildProps(){
  ...
  for(let i=0;i<8;i++){const x=-8+i*2.3,z=-37;mesh(new THREE.CylinderGeometry(.33,.43,2.7,12),stone,this.props,x,terrainHeight(x,z)+1.35,z);}
 ...
 update(view,progress,dt,paused,reduced){
  ...
  this.planet.position.x=this.ring.position.x=mobile?14:57;this.planet.scale.setScalar(mobile?.72:1);this.ring.scale.setScalar(mobile?.72:1);this.monument.position.x=mobile?-10:-26;
 ...
 setQuality(value){this.quality=value;for(const f of this.flowers)f.count=value==='low'?8000:16000;this.sun.castShadow=value!=='low';}
