...
import {meadowMaterial,wildflowerGeometry,wildflowerStemGeometry,buildUnderstory,buildGrove,buildFoothills} from './wander-scenery.js';
import {buildMeadowDetails,dressBench,clearPlantings} from './wander-details.js';
...
export class WanderWorld{
 constructor(renderer,camera){
  ...
  this.buildSky();this.buildTerrain();this.buildFlowers();this.buildTrees();buildFoothills(this.scene);this.buildProps();this.details=buildMeadowDetails(this.scene,this.wind);this.buildAvatar();this.loadAssets();
 }
 buildSky(){
  ...
  this.scene.add(new THREE.HemisphereLight(0xe3f0ff,0x697349,1.25));const sun=new THREE.DirectionalLight(0xffebc9,2.7);sun.position.set(-35,65,30);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);sun.shadow.camera.left=-48;sun.shadow.camera.right=48;sun.shadow.camera.top=38;sun.shadow.camera.bottom=-42;sun.shadow.camera.far=170;sun.shadow.bias=-.0003;sun.shadow.normalBias=.035;sun.target.position.set(0,0,-10);this.scene.add(sun,sun.target);this.sun=sun;
 ...
 buildTerrain(){
  ...
  for(let i=0;i<p.count;i++)p.setY(i,terrainHeight(p.getX(i),p.getZ(i)));geo.computeVertexNormals();this.ground=mesh(geo,meadowMaterial(this.wind),this.scene);this.ground.receiveShadow=true;
 ...
 buildFlowers(){
  ...
  const count=11000,petals=new THREE.InstancedMesh(flowerGeo,sway(material(0xffffff,{side:THREE.DoubleSide})),count),centers=new THREE.InstancedMesh(new THREE.SphereGeometry(.032,7,4).scale(1,.55,1).translate(0,.505,0),sway(material(0xb59646)),count),stems=new THREE.InstancedMesh(wildflowerStemGeometry(),sway(material(0x637b48,{side:THREE.DoubleSide})),count);
  const matrix=new THREE.Matrix4(),dummy=new THREE.Object3D(),rng=rand(843),color=new THREE.Color();let j=0;
  while(j<count){const x=(rng()-.5)*104,z=(rng()-.5)*104-12;if(pathDistance(x,z)<1.8||OBSTACLES.some(o=>Math.hypot(x-o.x,z-o.z)<o.r+1))continue;if(rng()>(.52+.48*Math.sin(x*.19)*Math.cos(z*.22)))continue;const scale=.45+rng()*.82;dummy.position.set(x,terrainHeight(x,z),z);dummy.rotation.set((rng()-.5)*.2,rng()*6.28,(rng()-.5)*.2);dummy.scale.setScalar(scale);dummy.updateMatrix();matrix.copy(dummy.matrix);petals.setMatrixAt(j,matrix);centers.setMatrixAt(j,matrix);stems.setMatrixAt(j,matrix);const patch=Math.sin(x*.18+z*.12);color.set((patch>.4?[0xf2e4c7,0xf7edd6,0xf5eddb,0xe3ceb4,0xf0ddb6]:patch<-.45?[0xdcbccb,0xf0d8d9,0xe8cfcb,0xc6b7d0,0xf1e4d4]:[0xf0dca1,0xe7c56e,0xf4e4bb,0xd9b877,0xf5e9cc])[Math.floor(rng()*5)]);petals.setColorAt(j,color);j++;}
 ...
 buildTrees(){
  this.leaves=buildGrove(this.scene,this.wind);
 }
 buildProps(){
  ...
  this.monument=new THREE.Group();this.monument.position.set(-26,terrainHeight(-26,-63),-63);this.monument.rotation.y=.36;this.monument.scale.setScalar(.78);this.scene.add(this.monument);mesh(new THREE.CylinderGeometry(3,3.5,3,24),stone,this.monument,0,1.5,0);
 ...
 buildAvatar(){
  ...
  this.shadow=mesh(new THREE.PlaneGeometry(1.35,1.35),new THREE.ShaderMaterial({transparent:true,depthWrite:false,vertexShader:'varying vec2 v;void main(){v=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',fragmentShader:'varying vec2 v;void main(){float r=length(v-.5)*2.;gl_FragColor=vec4(.15,.19,.10,(1.-smoothstep(.05,1.,r))*.25);}'}),this.scene);this.shadow.rotation.x=-Math.PI/2;
 }
 async loadAssets(){
  try{const {GLTFLoader}=await import('./vendor/three/addons/loaders/GLTFLoader.js');const loader=new GLTFLoader();
   ...
   loader.load('/assets/planet/daylight/environment.glb',g=>{const bench=g.scene.getObjectByName('bench'),bust=g.scene.getObjectByName('bust');if(bench){this.bench.clear();this.bench.add(bench.clone(true));dressBench(this.bench);}else dressBench(this.bench,.725,1);this.bench.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;}});if(bust){const b=bust.clone(true);b.position.set(0,3,0);b.scale.setScalar(11);this.monument.add(b);}this.assets.environment='ready';},undefined,()=>{dressBench(this.bench,.725,1);this.assets.environment='fallback';});
 ...
 setMemories(items){
  ...
  for(const item of items){let marker=this.markers.find(m=>m.item.id===item.id);if(marker){marker.item=item;continue;}const p=memoryPosition(item),group=new THREE.Group();group.position.set(p.x,terrainHeight(p.x,p.z),p.z);const blue=item.id==='living-room'?0x345a8b:item.demo?0xb96b3c:0x49685b,mat=material(blue),paper=material(0xffefd0);let shape;
   ...
   else{const profile=[[0,0],[.60,0],[.68,.055],[.65,.10],[.56,.34],[.54,.38],[0,.38]].map(([x,y])=>new THREE.Vector2(x,y));mesh(new THREE.LatheGeometry(profile,32),material(0xcfc6ac),group);mesh(new THREE.TorusGeometry(.575,.012,5,40),material(0xb59b61,{metalness:.35,roughness:.5}),group,0,.335,0).rotation.x=Math.PI/2;const letter=item.kind==='records';shape=mesh(new THREE.BoxGeometry(letter?.70:.52,.028,letter?.43:.68),paper,group,0,.415,0);shape.rotation.x=-.12;const pebble=mesh(new THREE.IcosahedronGeometry(letter?.075:.12,2),mat,group,letter?0:.20,.47,letter?0:-.15);pebble.scale.y=.45;}
   const halo=mesh(new THREE.TorusGeometry(.26,.009,6,36),new THREE.MeshBasicMaterial({color:blue,transparent:true,opacity:.52}),group,0,1.97,0);const dot=mesh(new THREE.SphereGeometry(.04,10,8),new THREE.MeshBasicMaterial({color:blue}),group,0,1.97,0);this.memoryGroup.add(group);marker={item,group,shape,halo,dot,p,age:0};this.markers.push(marker);if(item.id==='living-room-record'&&!this.reduced){this.wind.bloom.value.set(p.x,0,p.z);this.wind.growth.value=0;}
  }
  const clearings=this.markers.map(m=>({...m.p,radius:m.item.kind==='cases'?.65:1.05})),key=JSON.stringify(clearings);
  if(key!==this.clearingKey){clearPlantings([...this.flowers,this.grass,...this.details.plantings],clearings);this.clearingKey=key;}
 }
 ...
 update(view,progress,dt,paused,reduced){
  ...
  for(const m of this.markers){const d=Math.hypot(w.x-m.p.x,w.z-m.p.z);if(d<distance){this.nearest=m;distance=d;}m.halo.lookAt(this.camera.position);m.halo.rotation.z=this.time*.07;m.halo.material.opacity=d<4?.55:.24;m.halo.scale.setScalar(d<4?1.12:1);m.dot.position.y=1.97+(reduced?0:Math.sin(this.time*1.5+hashString(m.item.id)%10)*.08);}
  this.nearestDistance=distance;
  const mobile=this.width<=700,dist=(mobile?7.4:7.6)+(pose==='run'&&!reduced?.65:0),lift=2.0+w.pitch;
  this.planet.position.x=this.ring.position.x=mobile?18:66;this.planet.scale.setScalar(mobile?.55:.67);this.ring.scale.setScalar(mobile?.55:.67);this.monument.position.x=mobile?-10:-26;
  ...
  this.details.update(this.time,reduced,this.renderer.getPixelRatio());
 ...
 setQuality(value){this.quality=value;for(const f of this.flowers)f.count=value==='low'?6500:11000;this.grass.count=value==='low'?18000:32000;this.sun.castShadow=value!=='low';this.details.setQuality(value);}
