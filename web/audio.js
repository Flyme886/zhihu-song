...
export class PlanetAudio {
 ...
 tone(hz,duration,level=.2,pan=0,bus='effects'){
  const c=this.context,o=c.createOscillator(),g=c.createGain(),p=c.createStereoPanner();o.type='sine';o.frequency.setValueAtTime(hz,c.currentTime);o.frequency.exponentialRampToValueAtTime(hz*.995,c.currentTime+duration);g.gain.setValueAtTime(0,c.currentTime);g.gain.linearRampToValueAtTime(level,c.currentTime+.03);g.gain.exponentialRampToValueAtTime(.0001,c.currentTime+duration);p.pan.value=pan;o.connect(g);g.connect(p);p.connect(this[bus]);const item=this.track(o,g,duration,bus);o.onended=()=>{this.active.delete(item);o.disconnect();g.disconnect();p.disconnect();};
 }
 ...
 cue(name){
  if(!this.context||!this.settings.enabled||this.context.state!=='running')return;const t=this.context.currentTime;if(t-(this.lastCue[name]??-10)<.18)return;this.lastCue[name]=t;this.events.push({name,time:t});if(this.events.length>40)this.events.shift();
  ...
  else if(name==='pair'){this.tone(196,.7,.15,-.2);this.tone(293.66,1,.1,.2);}
 ...
 async suspend(){if(!this.context)return;this.cancelTravel();await this.context.suspend();}
