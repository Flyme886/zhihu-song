...
export function drawShareCard(canvas,record){
 ...
 const body=lines(ctx,record.note||'本次讨论未填写个人收获。',W-pad*2);ctx.font='600 32px "PingFang SC", "Microsoft YaHei", sans-serif';const title=lines(ctx,record.topicTitle,W-pad*2);
...
export function setupRecords({store,catalog,selectTopic,relations,renderTopics,closeTopics}){
 ...
 function list(){
  ...
  for(const record of records){const article=document.createElement('article');article.className='record-item';const h=document.createElement('h2');h.textContent=record.topicTitle;const time=document.createElement('small');time.textContent=new Date(record.savedAt).toLocaleString('zh-CN');const p=document.createElement('p');p.textContent=record.note||'保留了这次对话，还未填写新的想法。';article.append(time,h,p);
   ...
   add('导出',()=>{exported=structuredClone(record);$('#export-note').value=record.note||'';$('#export-markdown').value=recordMarkdown(exported);$('#export-status').textContent='编辑只影响本次导出；不会改写已保存记录。';drawShareCard($('#share-preview'),exported);$('#export-dialog').showModal();});
 ...
 $('#my-records').addEventListener('click',()=>{closeTopics();list();$('#records-dialog').showModal();});
 ...
 $('#export-note').addEventListener('input',()=>{if(exported){exported.note=$('#export-note').value;$('#export-markdown').value=recordMarkdown(exported);drawShareCard($('#share-preview'),exported);}});
 ...
 $('#export-md').addEventListener('click',()=>{if(exported)download(new Blob([$('#export-markdown').value],{type:'text/markdown;charset=utf-8'}),`思想引力场-${exported.topicId}.md`);});
}
