...
test('personal input stays personal across every local scenario',()=>{
 const person={id:'me',title:'我',author:'我',sourceKind:'personal',body:'我需要更多证据再做选择。',claim:'我需要更多证据再做选择。'};
 for(const topic of catalog)for(const condition of topic.scenarios)for(const turn of [0,2,4]){
  const text=demoTurn(person,topic.answers[0],turn,[condition.text],topic);
  assert.ok(text&&!text.includes('undefined'));assert.ok(!text.includes('从技术发展的角度看'));
 }
});
