...
const loads=new LatestRequest(),syncs=new LatestRequest();let switching=false,filter='all',pendingCondition='',loadInfo={};
...
function draw(now,dt){const p=travelProgress(now);if(stage==='travel'&&p>=1){stage='world';app.dataset.stage='world';nodes.forEach(n=>n.el.tabIndex=0);$('#announcer').textContent='已进入思想世界。可拖动画布、缩放，或选择星球阅读观点。';}
  ...
  for(const [i,n] of nodes.entries()){
    ...
    if((mine||p>.17)&&x+r>-15&&x-r<width+15&&y+r>-15&&y-r<height+15){
      ...
      n.label.style.opacity=mine||r>22&&match?'1':'0';
...
function enterWorld(text){
 ...
 relations.setInput(value,example);relations.refreshPicker();app.dataset.composing='false';$('#compose').textContent='＋ 补充我的想法';persist();
}
...
$('#profile').addEventListener('click',()=>{if(me.body)openDetail(me);else {app.dataset.composing='true';$('#knowledge').focus();}});
$('#edit-knowledge').addEventListener('click',()=>{$('#edit-form').hidden=false;$('#edit-text').value=me.body;$('#edit-text').focus();});
$('#edit-form').addEventListener('submit',e=>{e.preventDefault();const value=$('#edit-text').value.trim();if(!value){$('#edit-text').setCustomValidity('请保留至少一句知识或观点');$('#edit-text').reportValidity();return;}me.body=value;me.claim=value;relations.setInput(value,example);$('#detail-body').textContent=value;$('#edit-form').hidden=true;$('#announcer').textContent='我的知识已更新';$('#knowledge').value=value;relations.refreshPicker();persist();});
...
function pauseState(){const b=$('#pause');b.textContent=paused?'▷':'Ⅱ';b.setAttribute('aria-label',paused?'播放动画':'暂停动画');b.setAttribute('aria-pressed',String(paused));app.dataset.paused=String(paused);}pauseState();reducedMedia.addEventListener('change',e=>{reduced=e.matches;if(reduced){paused=true;pauseState();}});$('#pause').addEventListener('click',()=>{paused=!paused;pauseState();});
...
function saveRecord(record){store.addRecord(currentTopic.id,record);store.update(currentTopic.id,{discussion:null});renderTopics();$('#resume-discussion').hidden=true;}
function markRead(n){if(n===me)return;const state=store.get(currentTopic.id);if(!state.read.includes(n.id)){state.read.push(n.id);store.save();renderTopics();}}
...
async function selectTopic(id,{historyMode='push',force=false}={}){
 const next=catalog.find(t=>t.id===id)||catalog[0];if(!force&&next.id===currentTopic.id&&nodes.length>1){closeTopics();return;}
 persist();syncs.cancel();$('#sync-zhihu').disabled=false;const request=loads.begin();switching=true;stopMovement();relations.reset();
 ...
 if(historyMode!=='none'){const url=new URL(location.href);url.searchParams.set('topic',next.id);history[historyMode==='replace'?'replaceState':'pushState']({},'',url);}
 ...
 answers=styleAnswers(next.answers);loadInfo={};installNodes(session);populateTopic();renderTopics();closeTopics();switching=false;resize();start();
...
$('#discussion').addEventListener('click',()=>{if(!relations.discussion)persist();});addEventListener('pagehide',persist);addEventListener('popstate',()=>selectTopic(new URL(location.href).searchParams.get('topic'),{historyMode:'none'}));
$('#sync-zhihu').addEventListener('click',async()=>{
 const button=$('#sync-zhihu');if(button.disabled)return;const id=currentTopic.id,request=syncs.begin(),generation=syncs.token,controller=syncs.controller;button.disabled=true;$('#case-status').textContent='正在同步本题材料，最多两次搜索…';
 let timedOut=false;const timer=setTimeout(()=>{timedOut=true;controller.abort();},120000);
 try{const r=await fetch('/api/zhihu/sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({topicId:id}),signal:request.signal});const data=await r.json();if(!r.ok)throw Error(data.error||'同步失败');if(!request.current()||currentTopic.id!==id)return;if(!Array.isArray(data.answers)||data.answers.length<2||data.answers.some(a=>!a.id||typeof a.body!=='string'||typeof a.author!=='string'))throw Error('同步材料格式不完整，已保留当前材料。');
  persist();switching=true;relations.reset();answers=styleAnswers(data.answers);loadInfo=data;installNodes(store.get(id));populateTopic();switching=false;renderTopics();$('#case-status').textContent='已更新材料，已有讨论保留当时的来源快照。';
 }catch(e){if(syncs.token===generation&&currentTopic.id===id)$('#case-status').textContent=timedOut?'同步等待超时；保留原材料，稍后可查看服务缓存。':e.message;}
 finally{clearTimeout(timer);if(syncs.token===generation)button.disabled=false;}
});
...
setupRecords({store,catalog,selectTopic,relations,renderTopics,closeTopics});
closeTopics();
const requested=new URL(location.href).searchParams.get('topic');
await selectTopic(requested||store.data.lastTopic,{historyMode:'replace',force:true});
